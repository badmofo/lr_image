...
*/
#define        ReadOK(file,buffer,len) (fread(buffer, 1, len, file) != 0)
...
static void ReadImage (lr_image *im, FILE *fd, int len, int height, unsigned char (*cmap)[256], int interlace); /*1.4//, int ignore); */
...
lr_image *lr_read_image_gif(const char *filename)
{
  ...
  for (;;) {
    ...
    if (!(im = lr_allocate_image(imw, imh))) {
      fclose(fd);
      return 0;
    }
	...
	return 0;
      ...
      ReadImage(im, fd, imw, imh, localColorMap, BitSet(buf[8], INTERLACE)); 
    } else {
      ReadImage(im, fd, imw, imh, ColorMap, BitSet(buf[8], INTERLACE));
    }
    /*
    if (Transparent != (-1)) {
...
static void
ReadImage(lr_image *im, FILE *fd, int len, int height, unsigned char (*cmap)[256], int interlace) /*1.4//, int ignore) */
{
  unsigned char   c;      
  int             v;
